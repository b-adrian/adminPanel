...
class User_model extends CI_Model {
  ...
  public function updateGroups($postData=null, $action=null) {
    if ($action == "add") {
      ...
      if ($query->num_rows() > 0) {
        ...
        $sql2 = "INSERT INTO ". $this->rolesTable ." (name) VALUES (".$name.")";
        $this->db->query($sql2);
        return TRUE;
      }
    }
    if ($action == "edit") {
      ...
      if ($query->num_rows() > 0) {
        ...
        $sql2 = "UPDATE ". $this->rolesTable ." SET name = ".$name." WHERE id = ".$id;
        $this->db->query($sql2);
        return TRUE;
      }
    }
    if ($action == "delete") {
      ...
      $sql = "SELECT * FROM ". $this->usersTable ." WHERE role_id = ".$admin_group;
      $query = $this->db->query($sql);
      if ($query->num_rows() > 0) {
        return FALSE;
      } else {
        $sql2 = "DELETE FROM ". $this->rolesTable ." WHERE id = ".$admin_group;
        $this->db->query($sql2);
        return TRUE;
      }
    }
  ...
  public function getAdminGroups($additional="") {
    ...
    $sql = "SELECT * FROM ". $this->rolesTable ." ".$additional;
    $query = $this->db->query($sql);
    if ($query->num_rows() > 0) {
  ...
  public function getAdminInfo($adminid=null) {
    $sql = "SELECT * FROM ". $this->usersTable ." WHERE id = ".$this->db->escape(strip_tags((int)$adminid));
    $query = $this->db->query($sql);
    if ($query->num_rows() > 0) {
      return $query->row();
    } else {
  ...
  public function getAdmins() {
    $sql = "SELECT a.id, a.username, ag.name as 'role', a.name as 'fullname' FROM ". $this->usersTable ." a 
    LEFT JOIN ". $this->rolesTable ." ag ON a.role_id = ag.id";
    $query = $this->db->query($sql);
    if ($query->num_rows() > 0) {
      return $query->result_array();
    } else {
  ...
  public function updateAdmins($postData=null, $action=null) {
    if ($action == "add") {
      ...
      if ($query->num_rows() > 0) {
        ...
        $sql2 = "INSERT INTO admin (username,password,email,created_date,verification_key,role_id,name,address,address2,city,state,zip) VALUES ($username, $password, $email, $now, $verification_key, $admin_group, $name, $address, $address2, $city, $state, $zip)";
        $this->db->query($sql2);
        return TRUE;   
      }
    ...
    if ($action == "edit") {
      ...
      if ($query->num_rows() > 0) {
        if ($pass == 0) {
          $sql = "UPDATE ". $this->usersTable ." SET email = $email, name = $name, role_id = $admin_group, address = $address, address2 = $address2, city = $city, state = $state, zip = $zip WHERE id = ".$this->db->escape($query->row()->id);
          $this->db->query($sql);
          return TRUE;
        } else {
          ...
          $sql = "UPDATE ". $this->usersTable ." SET email = $email, name = $name, role_id = $admin_group, address = $address, address2 = $address2, city = $city, state = $state, zip = $zip, password = $password WHERE id = ".$this->db->escape($query->row()->id);
          $this->db->query($sql);
          return TRUE;
        }
      } else {
    ...
    if ($action == "delete") {
      ...
      if ((int)$postData["id"] == $this->session->userdata("admin_id")) { 
       ...
       $sql = "DELETE FROM ". $this->usersTable ." WHERE id = ".$admin_id;
       $this->db->query($sql);
       return TRUE;     
      }
  ...
  public function adminLogin($postData) {
  	...
  	$sql = "SELECT * FROM ". $this->usersTable ." WHERE username = ".$username." AND password = ".$password;
  	$query = $this->db->query($sql);
  	if ($query->num_rows() > 0) {
  		$q = $query->row();
  		...
  		$ip = $this->getUserIP();
  		$sql2 = "UPDATE ". $this->usersTable ." SET last_signin = NOW(), ip = ".$this->db->escape($ip)." WHERE id = ".$q->id;
  		$this->db->query($sql2);
  		return TRUE;
  	} else {
  		return 2;
  ...
  public function verifyUser() {
  	if ($this->session->userdata("username") && $this->session->userdata("verification_key") && $this->session->userdata("admin_id") && $this->session->userdata("loggedin")) {
  		$sql = "SELECT * FROM ". $this->usersTable ." WHERE id = ".$this->db->escape(strip_tags((int)$this->session->userdata("admin_id")))." AND verification_key = ".$this->db->escape(strip_tags($this->session->userdata("verification_key")))." AND username = ".$this->db->escape(strip_tags($this->session->userdata("username")));
  		$query = $this->db->query($sql);
  		if ($query->num_rows() > 0) {
  			return TRUE;
  		} else {
